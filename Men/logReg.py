...
print(scores)
# print("%s: %.2f%%" % (logReg.metrics_names[1], scores[1]*100))
# print("%s: %.2f%%" % (logReg.metrics_names[0], scores[0]*100))
# input('Press button to continue')

y_pred = clf.predict_proba(X_test)
LL = log_loss(y_test, y_pred[:,1])
print(LL)
'''-------------- Make prediction -------------------------'''
data_dir = './'
...
if save_model == 'y':
    ...
    model_json = logReg.to_json()
    with open("logReg.json", "w") as json_file:
        json_file.write(model_json)
        # serialize weights to HDF5
    logReg.save_weights("logReg.h5")
